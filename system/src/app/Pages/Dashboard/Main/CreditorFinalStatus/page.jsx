...
function RouteList() {
  ...
  const handleSearch = () => {
    ...
    const filtered = originalData.filter(entry => {
      ...
      // Filter based on included and excluded supplier ids
      ...
      // Log details of route matching
      console.log('Entry Details:', entry.details);  // Log the full entry details to check structure
      const routeIds = detailsForPurchase.map(d => {
        const routeId = Number(d.route_id);
        console.log(`Route ID from detail: ${routeId}`);  // Log each route_id to identify the issue
        return routeId;
      });
  
      // Filter based on included and excluded route ids
      const routeIncluded =
      ...
      console.log('Route Included:', routeIncluded);  // Log if the route is included in the current filter
      console.log('Route Excluded:', routeExcluded);  // Log if the route is excluded from the current filter
