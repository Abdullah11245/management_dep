...
const ItemsTable = () => {
    ...
    const fetchItems = async () => {
      try {
        ...
        if (response.data && response.data) {
          setItems(response.data); // Assuming 'items' is the key in response
        }
      } catch (error) {
  ...
  const handleEdit = (item) => {
    router.push(`/Pages/Dashboard/ItemList/${item.id}`); // Redirect to edit page with item ID
  };
  if ( isLoading ) return <div className="flex justify-center items-center h-screen bg-white">
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex justify-between items-center mt-4">
        <span className="text-sm font-semibold text-gray-700">
          Showing 1 to 200 of {items.length} entries
        </span>
